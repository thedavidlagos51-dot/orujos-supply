...
function productForm(p = null) {
  ...
  backProducts.onclick = () => shell('products'); saveProduct.onclick = () => saveProductForm(p?.id);
  if (p) toggleProduct.onclick = () => { pActive.value = p.active ? 'false' : 'true'; saveProductForm(p.id); };
}
async function saveProductForm(id) {
  const payload = { code:pCode.value.trim(), name:pName.value.trim(), category_id:pCategory.value ? Number(pCategory.value) : null, subcategory:pSubcategory.value.trim() || null, unit:pUnit.value.trim(), unit_price:Number(pPrice.value), supplier:pSupplier.value.trim() || null, notes:pNotes.value.trim() || null, active:pActive.value === 'true' };
