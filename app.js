...
async function loadProducts() { products = await query(db.from('products').select('*, categories(name), product_scopes(branch_id,area_id)').eq('active', true).order('name')); }
function shell(view) {
...
function scoped(p, branchId, areaId) { return !p.product_scopes?.length || p.product_scopes.some(s => s.branch_id === Number(branchId) && s.area_id === Number(areaId)); }
...
async function productsView() { await loadProducts(); content.innerHTML = `<section class="panel"><h1>Productos</h1><p class="muted">Administración de catálogo y precios.</p><div class="table-wrap"><table><thead><tr><th>Código</th><th>Producto</th><th>Categoría</th><th>Unidad</th><th>Precio</th><th>Estado</th></tr></thead><tbody>${products.map(p => `<tr><td>${esc(p.code)}</td><td>${esc(p.name)}</td><td>${esc(p.categories?.name)}</td><td>${esc(p.unit)}</td><td>${money(p.unit_price)}</td><td>${p.active ? 'Activo' : 'Inactivo'}</td></tr>`).join('')}</tbody></table></div><p class="muted">Los cambios de catálogo se realizan desde Supabase mientras se completa el módulo de edición.</p></section>`; }
